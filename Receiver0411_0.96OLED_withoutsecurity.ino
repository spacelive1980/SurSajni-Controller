...
#include <ESPmDNS.h> // NEW: For OTA hostname resolution
...
bool pumpRunning = false;    // True if relay is HIGH
...
void updateLiveTurbidityValue() {
    ...
    if (millis() - lastTurbiditySampleTime >= TURBIDITY_SAMPLE_INTERVAL_MS) {
        lastTurbiditySampleTime = millis();
        turbiditySampleSum += analogRead(TURBIDITY_SENSOR_PIN);
    ...
    if (wifiMode == WIFI_STA_MODE && WiFi.status() == WL_CONNECTED) {
      ...
      if (success) {
        ...
        if (isRtcWorking && abs(long(rtc.now().unixtime() - epochTime)) > 10) {
          ...
          Serial.printf("NTP Task: RTC time synchronized. New time: %s\n", timeClient.getFormattedTime().c_str());
        } else if (isRtcWorking) {
          Serial.println("NTP Task: RTC time is already accurate. No sync adjustment needed.");
        }
        ...
        Serial.println("NTP Task: NTP update failed. Will retry later.");
      }
      
      // After an attempt (successful or not), sleep for the long interval.
      Serial.printf("NTP Task: Sleeping for %lu ms.\n", NTP_SYNC_INTERVAL_MS);
...
void readTurbiditySensor() {
    ...
    if (turbidityValue > turbidityLimit) {
        if (!isWaterDirty) {
            ...
            isWaterDirty = true;
            dirtyWaterDetectedTime = millis();
            Serial.printf("High turbidity detected (%d > %d). Starting timeout.\n", turbidityValue, turbidityLimit);
        } else {
            // Water is already known to be dirty, check if the timeout has expired.
            if (millis() - dirtyWaterDetectedTime > (turbidityTimeoutSec * 1000UL)) {
                ...
                if (!isDirtyWaterError && !retryInProgress) {
                    ...
                    Serial.println("Dirty water timeout detected.");
                    ...
                    if (retryLogicEnabled) {
                        if (remainingDryRunAttempts > 0) {
                            ...
                            retryStartTime = millis(); // Record when the retry interval begins.
                            remainingDryRunAttempts--; // Use one attempt.
                            Serial.printf("Retrying due to turbidity... Attempts left: %d\n", remainingDryRunAttempts);
                            ...
                            Serial.println("Turbidity error: All retry attempts exhausted.");
                            if (buzzerEnabled && !isDirtyWaterBeepActive) {
                                beepForDirtyWater();
                            }
                        }
                        ...
                        sensorError = SENSOR_ERROR_DIRTY_WATER;
                        snprintf(sensorErrorDetailMessage, sizeof(sensorErrorDetailMessage), "Turbidity: %d", turbidityValue);
                        Serial.println("Turbidity error: Retry logic is disabled.");
                        if (buzzerEnabled && !isDirtyWaterBeepActive) {
                            beepForDirtyWater();
                        }
                    }
    ...
    if (!pumpRunning && !waitingForPumpRelay && !dryRunError && !retryInProgress && 
        ...
        if (waterIsPresent) {
            ...
            if (waterPresentStartTime == 0) {
                waterPresentStartTime = millis();
                Serial.printf("Water detected! Turbidity (%d) > threshold (%d). Starting %ds start timer.\n", turbidityValue, waterPresenceThreshold, waterSensingStartDelaySec);
            } else if (millis() - waterPresentStartTime >= (waterSensingStartDelaySec * 1000UL)) {
                // Timer expired, it's safe to start the pump.
                Serial.printf("%ds start delay passed. Initiating pump start due to water presence.\n", waterSensingStartDelaySec);
                ...
                if (dryRunError) {
                    ...
                    dryRunResetMessageStartTime = millis();
                    Serial.println("Water Sensing start is due. Automatically resetting DRY RUN error.");
                }
    ...
    if (pumpRunning) { // This logic should only apply when the pump is actually on
        if (!waterIsPresent) {
            ...
            if (waterAbsentStartTime == 0) {
                waterAbsentStartTime = millis();
                Serial.printf("Water NOT detected! Turbidity (%d) <= threshold (%d). Starting %ds stop timer.\n", turbidityValue, waterPresenceThreshold, waterSensingStopDelaySec);
            } else if (millis() - waterAbsentStartTime >= (waterSensingStopDelaySec * 1000UL)) {
                // Timer expired, it's safe to stop the pump.
                Serial.printf("%ds stop delay passed. Stopping pump due to water absence.\n", waterSensingStopDelaySec);
                stopPump();
...
bool debounceButton(uint8_t pin, ButtonState& btn) {
    ...
    if ((millis() - btn.lastChangeTime) > DEBOUNCE_INTERVAL_MS) {
        if (btn.lastStableState != currentState) {
            btn.lastStableState = currentState;
            // If the new stable state is pressed (LOW), it's a new press event
            if (btn.lastStableState == LOW) {
                isNewlyPressed = true;
    ...
    btn.isPressed = (btn.lastStableState == LOW);
...
void startBuzzerContinuous(unsigned long duration_ms) {
    ...
    buzzerActiveUntil = millis() + duration_ms;
    currentBuzzerState = BUZZER_CONTINUOUS;
    isBuzzerCurrentlyOn = true;
    Serial.printf("Continuous buzzer started for %lu ms.\n", duration_ms);
}
...
void startBuzzerSequence(uint8_t count, unsigned long on_duration_ms, unsigned long off_duration_ms) {
    ...
    Serial.printf("Buzzer sequence started: %d beeps, %lu on, %lu off.\n", count, on_duration_ms, off_duration_ms);
}
...
void startBriefBeep() {
    ...
    if (currentBuzzerState == BUZZER_BRIEF && millis() < buzzerActiveUntil) return; // Already beeping briefly

    ledc_set_duty(LEDC_LOW_SPEED_MODE, BUZZER_LEDC_CHANNEL, buzzerVolume);
    ledc_update_duty(LEDC_LOW_SPEED_MODE, BUZZER_LEDC_CHANNEL);
    buzzerActiveUntil = millis() + 50; // 50ms brief beep
    currentBuzzerState = BUZZER_BRIEF;
    isBuzzerCurrentlyOn = true;
    Serial.println("Brief beep started.");
}
        ...
        case BUZZER_SEQUENCE:
            if (currentBeepCount > 0) {
                if (isBuzzerCurrentlyOn) {
                    if (millis() - buzzerToggleTime >= beepOnDuration) {
                        ledc_set_duty(LEDC_LOW_SPEED_MODE, BUZZER_LEDC_CHANNEL, 0);
                        ledc_update_duty(LEDC_LOW_SPEED_MODE, BUZZER_LEDC_CHANNEL);
                        isBuzzerCurrentlyOn = false;
                        buzzerToggleTime = millis();
                    ...
                    if (millis() - buzzerToggleTime >= beepOffDuration) {
                        ledc_set_duty(LEDC_LOW_SPEED_MODE, BUZZER_LEDC_CHANNEL, buzzerVolume);
                        ledc_update_duty(LEDC_LOW_SPEED_MODE, BUZZER_LEDC_CHANNEL);
                        isBuzzerCurrentlyOn = true;
                        buzzerToggleTime = millis();
...
void loadSettingsFromEEPROM() {
    ...
    if (EEPROM.read(EEPROM_MAGIC_ADDR) != EEPROM_MAGIC_VALUE) { 
      Serial.println("EEPROM not initialized, using default settings.");
    ...
    /*
    if (ap_password[0] == '\0') {
        Serial.println("WARN: Loaded AP password was empty. Reverting to default.");
    ...
    */
    ...
    if (http_password[0] == '\0' || http_password[0] == (char)0xFF) {
        Serial.println("WARN: Loaded HTTP password was invalid. Reverting to default.");
...
bool initiatePumpStart(bool isScheduledRun) {
    ...
    if (currentProfile == PROFILE_LEVEL_INDICATOR) {
        Serial.println("Pump start blocked: Level Indicator profile is active.");
        pumpBlockedMessageActive = true;
        pumpBlockedMessageStartTime = millis();
        return false;
    }
    ...
    if ( (dryRunError && !isScheduledRun) || waitingForPumpRelay || pumpRunning || (!isScheduledRun && sensorError != SENSOR_ERROR_NONE && !sensorErrorBypassEnabled)) {
      if (dryRunError && !isScheduledRun) { // MODIFIED: Only log/block if it's NOT a scheduled run.
          Serial.println("Pump start blocked: Dry Run Error is active. Please reset.");
          // Optionally trigger the pump blocked message on the display
          pumpBlockedMessageActive = true;
          pumpBlockedMessageStartTime = millis();
      } else {
          Serial.println("Pump start blocked due to an existing run or an un-bypassed sensor error.");
      }
      return false;
    }
    ...
    if (dryRunLogicEnabled) {
        ...
        if (currentProfile == PROFILE_MONOBLOCK) {
            Serial.println("Monoblock Profile: Starting pump directly (no priming).");
            digitalWrite(RELAY_PIN, HIGH); // Turn ON pump relay directly
            pumpRunning = true;
            ...
            Serial.printf("Initiating pump start sequence (%dms valve delay).\n", primingTimeMs);
            Serial.println("-> Valve relay (pin 17) setting to HIGH (ON).");
            digitalWrite(VALVE_RELAY_PIN, HIGH); // Turn ON valve relay (high-triggered)
            valveRelayStartTime = millis();
    ...
    // remainingDryRunAttempts is NOT reset here. It's reset on successful flow or manual reset.
        ...
        Serial.println("Dry run logic disabled. Starting pump directly.");
        Serial.println("-> Pump relay (pin 16) setting to HIGH (ON).");
...
void stopPump() {
    ...
    digitalWrite(VALVE_RELAY_PIN, LOW); // Ensure valve relay is OFF
    pumpRunning = false;
    ...
    Serial.println("-> Pump relay (pin 16) set to LOW (OFF).");
    Serial.println("-> Valve relay (pin 17) set to LOW (OFF).");
}
...
void handleMenuButtons() {
    ...
    if (factoryResetConfirmActive) {
        if (millis() - factoryResetConfirmStartTime > 10000) { // 10 second timeout
    ...
    if (inSettingsMenu && setBtn.isPressed && !setPressed && (millis() - setBtn.pressStartTime >= BUTTON_LONG_PRESS_THRESHOLD_MS)) {
        inSettingsMenu = false;
        editingSetting = false;
        menuHistoryPointer = 0;
        currentCalibrationState = CAL_IDLE;
        editingScheduleIndex = 0;
        Serial.println("Long SET press: Exiting settings menu to home screen.");
        saveSettingsToEEPROM();
        ignoreButtonsUntil = millis() + 500; // Ignore for 500ms to prevent re-entry
    ...
    if (!inSettingsMenu) {
        if (setPressed) {
            ...
            if (currentProfile == PROFILE_LEVEL_INDICATOR) {
                pumpBlockedMessageActive = true;
                pumpBlockedMessageStartTime = millis();
                Serial.println("PUMP BLOCKED: Manual toggle attempted in Level Indicator profile.");
                return;
            }
            if (sensorError != SENSOR_ERROR_NONE && !sensorErrorBypassEnabled) {
                pumpBlockedMessageActive = true;
                pumpBlockedMessageStartTime = millis();
                Serial.println("PUMP BLOCKED: Sensor Error active and bypass disabled.");
                return;
            }
            if (pumpMode == MANUAL) {
                ...
                if (downPressed && (pumpRunning || waitingForPumpRelay)) {
                    ...
                    pumpBlockedMessageActive = true;
                    pumpBlockedMessageStartTime = millis();
                    Serial.println("PUMP BLOCKED: Manual START not allowed in SCHEDULE mode.");
                }
            }
        }
    ...
    if (editingSetting) {
        ...
        if (setPressed) {
            ...
            if (currentMenuPage == PAGE_SCHEDULE_EDIT && !isScheduleValid(schedules[editingScheduleIndex])) {
                scheduleInvalidMessageActive = true; scheduleInvalidMessageStartTime = millis();
        ...
        } else if ((upBtn.isPressed || downBtn.isPressed) && (millis() - (upBtn.isPressed ? upBtn.pressStartTime : downBtn.pressStartTime) > BUTTON_REPEAT_DELAY_MS)) {
            if (millis() - lastRepeatTime > BUTTON_REPEAT_RATE_MS) {
                applyChange = true;
                lastRepeatTime = millis();
            }
        }
            ...
            if (upPressed) {
                ...
                if (currentItem.type == MENU_ITEM_SUBMENU) {
                    ...
                    if (currentMenuPage == PAGE_SCHEDULES_MENU && currentItem.settingIndex == 100) {
                        ...
                        schedulesClearedMessageActive = true; schedulesClearedMessageStartTime = millis();
                        ...
                        singleScheduleClearedMessageActive = true; singleScheduleClearedMessageStartTime = millis();
                    ...
                    } else if (currentItem.settingIndex == 100) { factoryResetConfirmActive = true; factoryResetConfirmStartTime = millis();
                    }
                }
            }
        }
...
void updateDisplay() {
    ...
    if (scheduleInvalidMessageActive) {
      if (millis() - scheduleInvalidMessageStartTime < SCHEDULE_INVALID_DISPLAY_DURATION_MS) {
        display.setFont(u8g2_font_7x13_tf);
    ...
    if (sensorError != SENSOR_ERROR_NONE) {
        if ((millis() / 5000) % 2 == 0) { // On for 5 seconds
    ...
    if (pumpBlockedMessageActive) {
      if (millis() - pumpBlockedMessageStartTime < PUMP_BLOCKED_DISPLAY_DURATION_MS) {
    ...
    if (dryRunResetMessageActive) {
      if (millis() - dryRunResetMessageStartTime < ERROR_RESET_MESSAGE_DURATION_MS) {
        display.setFont(u8g2_font_7x13_tf);
    ...
    if (sensorErrorResetMessageActive) {
      if (millis() - sensorErrorResetMessageStartTime < ERROR_RESET_MESSAGE_DURATION_MS) {
        display.setFont(u8g2_font_7x13_tf);
    ...
    if (singleScheduleClearedMessageActive) {
      if (millis() - singleScheduleClearedMessageStartTime < SCHEDULE_CLEARED_DISPLAY_DURATION_MS) {
    ...
    if (schedulesClearedMessageActive) {
      if (millis() - schedulesClearedMessageStartTime < SCHEDULE_CLEARED_DISPLAY_DURATION_MS) {
    ...
    if (limitSetMessageActive) {
      if (millis() - limitSetMessageStartTime < SCHEDULE_CLEARED_DISPLAY_DURATION_MS) { // reuse duration
        display.setFont(u8g2_font_6x12_tr);
        char msg[20];
      ...
      switch (screenState) {
        ...
        case 3: // Dry run countdown (only if not MANUAL or if manualModeSafetyLogicEnabled)
          ...
          if (!dryRunLogicEnabled || (pumpMode == MANUAL && !manualModeSafetyLogicEnabled)) {
            ...
            if (waitingForFlow) {
              uint32_t remaining = (dryRunDelayMin * 60) - ((millis() - pumpStartTime) / 1000);
    ...
    if (currentMenuPage == PAGE_CALIBRATION && currentCalibrationState != CAL_IDLE) {
        ...
        switch (currentCalibrationState) {
            case CAL_CLEAN_WATER_SAMPLING: {
                ...
                unsigned long progress = ((millis() - calibrationStartTime) * 100) / CALIBRATION_SAMPLE_DURATION_MS;
...
void handleResetDryRunWrapped() {
    ...
    dryRunResetMessageStartTime = millis(); // NEW: Set message start time
    xSemaphoreGive(sharedDataMutex);
    Serial.println("Dry run error manually reset via Web UI.");
...
void handleUpdateAllSettingsWrapped() {
    ...
    if (server.hasArg("action")) {
        ...
        if (action == "clear_schedules") {
            ...
            Serial.println("All schedules cleared via Web UI.");
        ...
        if (action == "clear_schedule" && server.hasArg("index")) {
            ...
            if (index < MAX_SCHEDULES) {
                ...
                Serial.printf("Schedule #%d cleared via Web UI.\n", index + 1);
    ...
    if (server.hasArg("startSensorLevel") && server.hasArg("endSensorLevel")) { // NEW: Handle Start/End sensor levels
        ...
        if (newStart < newEnd && newStart <= TANK_75 && newEnd >= TANK_25) {
            ...
            Serial.println("WebUI sent invalid start/end sensor levels. Ignoring.");
        }
    }
...
void handleResetSensorErrorWrapped() {
    ...
    sensorErrorResetMessageStartTime = millis(); // NEW: Set message start time
    Serial.println("Sensor error manually reset.");
...
void handleTogglePumpWrapped() {
    ...
    if (sensorError != SENSOR_ERROR_NONE && !sensorErrorBypassEnabled) {
        ...
        pumpBlockedMessageActive = true;
        pumpBlockedMessageStartTime = millis();
        Serial.println("PUMP BLOCKED: Manual toggle attempted via Web UI due to sensor error.");
...
void handleScreenRotation() {
    ...
    if (millis() - lastScreenSwitch >= DRY_RUN_DISPLAY_ROTATION_MS) {
...
void handleLEDs() {
    ...
    digitalWrite(HEARTBEAT_LED_PIN, heartbeatExpired ? (millis() / 500) % 2 : HIGH); // Blinks if expired, solid ON if active
    ...
    digitalWrite(BATTERY_LED_PIN, lowBattery ? (millis() / 500) % 2 : LOW);
}
...
void checkDryRunTimeout() {
    ...
    if (pumpRunning && waitingForFlow) {
      ...
      if (millis() - pumpStartTime > dryRunDelayMin * 60000UL) {
        stopPump(); // Stop both relays
        Serial.println("Dry run timeout detected.");
        // NEW: Check if retry logic is enabled before starting the retry process
        if (retryLogicEnabled) {
          if (remainingDryRunAttempts > 0) {
            retryInProgress = true; // Start retry process
            retryStartTime = millis(); // Record retry start time
            remainingDryRunAttempts--; // Decrement remaining attempts
            Serial.print("Retrying... Attempts left: "); Serial.println(remainingDryRunAttempts);
          } else {
            dryRunError = true; // All retry attempts exhausted, set dry run error
            retryInProgress = false; // No longer retrying
            Serial.println("Dry run error: All attempts exhausted.");
          ...
          dryRunError = true;
          Serial.println("Dry run error: Retry logic is disabled.");
...
void checkRetryTrigger() {
    ...
    if (retryInProgress && !pumpRunning) { // If a retry is pending and pump is currently off
      if (millis() - retryStartTime > retryIntervalMin * 60000UL) {
...
void checkDryRunReset() {
    ...
    if (dryRunError) {
        ...
        if (pumpMode != MANUAL || manualModeSafetyLogicEnabled) {
            ...
            if (upBtn.isPressed && (millis() - upBtn.pressStartTime >= BUTTON_HOLD_TIME_MS)) {
                dryRunError = false; // Clear dry run error
                isDryRunErrorBeepActive = false; // Reset the beep flag
                remainingDryRunAttempts = dryRunAttempts; // Reset attempts
                dryRunResetMessageActive = true; // NEW: Trigger display message
                dryRunResetMessageStartTime = millis(); // NEW: Set message start time
                Serial.println("Dry run error manually reset.");
    ...
    if (sensorError != SENSOR_ERROR_NONE) {
        ...
        if (downBtn.isPressed && (millis() - downBtn.pressStartTime >= BUTTON_HOLD_TIME_MS)) {
...
void sendAcknowledgment(uint16_t senderID) {
  ...
  Serial.println("------------------------------------");
  Serial.println("Preparing to send ACK to transmitter...");
  Serial.printf("  -> Target Sender ID: 0x%04X\n", senderID);
  Serial.printf("  -> Sending DESIRED TX Power: %d\n", transmitterTxPower);
  Serial.printf("  -> Sending DESIRED ACK Attempts: %d\n", transmitterAckAttempts);
  Serial.printf("  -> Sending RTC Unix Time: %lu\n", unixTime);
  Serial.println("------------------------------------");
  // --- END of new logging ---
  ...
  LoRa.receive();
  Serial.print("ACK sent with RTC time (Unix: ");
  Serial.print(unixTime);
  Serial.print(") and TX Power (");
  Serial.print(transmitterTxPower);
  Serial.print(") and ACK Attempts ("); // NEW
  Serial.print(transmitterAckAttempts); // NEW
  Serial.print(") for ID: 0x");
...
void webUiTask(void *pvParameters) {
  Serial.println("Web UI Task started on Core 0.");
...
void setup() {
    ...
    digitalWrite(RELAY_PIN, LOW); // Important: Keep pump off on startup
    // For a high-triggered relay, LOW is OFF.
    // For a low-triggered relay, HIGH is OFF.
    digitalWrite(VALVE_RELAY_PIN, LOW); // Set relay to OFF on startup
    Serial.println("Valve relay pin configured as high-triggered.");

    // --- NEW: RX LED Hardware Test ---
    Serial.println("--- Starting RX LED Hardware Test ---");
    for (int i = 0; i < 5; i++) {
    ...
    Serial.println("--- RX LED Hardware Test Finished ---");
    ...
    if (wifiMode == WIFI_AP_MODE) {
        ...
        Serial.printf("Setting up WiFi STA, connecting to '%s'...\n", staSsid);
        WiFi.mode(WIFI_STA);
    ...
    if (!isWebRequestActive) {
      ...
      if (wifiMode == WIFI_STA_MODE) {
          if (WiFi.status() != WL_CONNECTED) {
              ...
              if (millis() - lastWifiConnectAttempt > WIFI_RECONNECT_INTERVAL_MS) {
                  Serial.println("WiFi STA not connected, attempting reconnect...");
                  connectToWiFiSTA();
              }
      ...
      if (waitingForPumpRelay && !pumpRunning && (millis() - valveRelayStartTime >= primingTimeMs)) {
        Serial.println("Priming time elapsed. Starting pump.");
        digitalWrite(VALVE_RELAY_PIN, LOW); // Turn valve relay OFF
        digitalWrite(RELAY_PIN, HIGH);       // Turn pump relay ON
        pumpRunning = true;
        pumpStartTime = millis();
        if (pumpMode != MANUAL || manualModeSafetyLogicEnabled) { waitingForFlow = true; }
        flowOk = false;
        waitingForPumpRelay = false;
        Serial.println("Main pump relay activated.");
      }
      ...
      if (isRtcWorking) {
        if (lastCheckDay == 0) { // First run initialization
          ...
          Serial.println("New day detected. Resetting daily schedule triggers.");
          for (uint8_t i = 0; i < MAX_SCHEDULES; i++) {
      ...
      if (inSettingsMenu) {
          ...
          for (uint8_t i = 0; i < MAX_SCHEDULES; i++) {
            ...
            if (!scheduleTriggeredToday[i] &&
              ...
              if (shouldTrigger) {
                ...
                if (dryRunError) {
                    Serial.println("Scheduled run is due. Automatically resetting Dry Run Error.");
                    dryRunError = false;
                ...
                if (isDirtyWaterError) {
                    Serial.println("Scheduled run is due. Automatically resetting Turbidity Error.");
                    isDirtyWaterError = false;
                    ...
                    sensorErrorResetMessageStartTime = millis();
                    ...
                    remainingDryRunAttempts = dryRunAttempts; 
                    Serial.println("Retry attempts reset for new scheduled run.");
                    // --- END BUG FIX ---
                }
                // --- End of NEW block ---
                ...
                if (topSensorFault) {
                    Serial.println("Scheduled run blocked: Critical top sensor fault.");
                    pumpBlockedMessageActive = true;
                    pumpBlockedMessageStartTime = millis();
                } else {
                ...
                if (pumpWasInitiated) {
                  ...
                  if (schedules[i].repeatType == SPECIFIC_DATE || schedules[i].repeatType == NO_REPEAT) {
                      Serial.printf("One-time schedule #%d triggered. Clearing it.\n", i + 1);
                      schedules[i] = {0, 0, EVERY_DAY, 0, 0, 0};
                      saveSettingsToEEPROM();
                  }
                }
        ...
        if (pumpRunning && scheduleDurationMin > 0) {
          if (millis() - pumpStartTime >= scheduleDurationMin * 60000UL) {
            Serial.println("Pump turning OFF due to scheduled duration.");
            stopPump();
            if (currentPumpRunHadFlow) remainingDryRunAttempts = dryRunAttempts;
          }
        }
...
void handleAutoDryRunReset() {
    ...
    if (waterIsPresent) {
        ...
        if (autoResetWaterPresentStartTime == 0) {
            autoResetWaterPresentStartTime = millis();
            Serial.println("Dry Run Error active & Water Detected. Starting auto-reset confirmation timer.");
        } else if (millis() - autoResetWaterPresentStartTime >= (waterSensingStartDelaySec * 1000UL)) {
            // The confirmation time has passed, and water is still present. Reset the error.
            Serial.println("Auto-reset timer expired. Resetting Dry Run Error now.");
            dryRunError = false;
            isDryRunErrorBeepActive = false; // Stop any ongoing error beeps.
            remainingDryRunAttempts = dryRunAttempts; // Restore all retry attempts.
            dryRunResetMessageActive = true; // Show a confirmation message on the display.
...
void handleAutoTurbidityReset() {
    ...
    if (!turbidityErrorWaterAbsent) {
        if (!waterIsPresent) {
            ...
            Serial.println("Turbidity Error active & Water Disappeared. Waiting for water to return to reset.");
        }
    } 
    // State 2: Error is active, water has disappeared, now waiting for it to reappear.
        ...
        if (waterIsPresent) {
            ...
            Serial.println("Water has returned. Auto-resetting Turbidity Error.");
...
void startCleanWaterCalibration() {
    ...
    Serial.println("Starting clean water calibration...");
}
...
void startDirtyWaterCalibration() {
    ...
    Serial.println("Starting dirty water calibration...");
}
...
void processCalibration() {
    ...
    if (currentCalibrationState == CAL_CLEAN_WATER_SAMPLING || currentCalibrationState == CAL_DIRTY_WATER_SAMPLING) {
        if (millis() - calibrationStartTime < CALIBRATION_SAMPLE_DURATION_MS) {
            // Still sampling
            calibrationSampleSum += analogRead(TURBIDITY_SENSOR_PIN);
            ...
            if (calibrationSampleCount > 0) {
                ...
                if (currentCalibrationState == CAL_CLEAN_WATER_SAMPLING) {
                    ...
                    Serial.printf("Clean water calibration finished. Avg value: %d\n", calibratedCleanValue);
                } else { // CAL_DIRTY_WATER_SAMPLING
                    calibratedDirtyValue = avgValue;
                    Serial.printf("Dirty water calibration finished. Avg value: %d\n", calibratedDirtyValue);
                }
                ...
                Serial.printf("New turbidity limit automatically set to: %d\n", turbidityLimit);
            }
...
void handleStartEndSensorLogic() {
    ...
    if ((pumpRunning || waitingForPumpRelay) && (pumpMode != MANUAL || manualModeSafetyLogicEnabled)) {
        if (currentTankLevel >= endSensorLevel && currentTankLevel != TANK_INVALID) { // TANK_INVALID check is a failsafe
            Serial.printf("Pump turning OFF: Reached End Sensor Level (%s).\n", getTankLevelString(endSensorLevel));
            stopPump();
    ...
    if (!pumpRunning && !waitingForPumpRelay && !dryRunError && (sensorError == SENSOR_ERROR_NONE || sensorErrorBypassEnabled)) {
        ...
        if (pumpMode == AUTO && currentTankLevel <= startSensorLevel) {
             Serial.printf("Pump turning ON: Reached Start Sensor Level (%s).\n", getTankLevelString(startSensorLevel));
...
void loraTask(void *pvParameters) {
  Serial.println("LoRa Task started and pinned to Core 0.");
  ...
  for (;;) { // Infinite loop for this task
    ...
    if (rxLedOnTime_task > 0 && millis() - rxLedOnTime_task > 100) {
        digitalWrite(RX_LED_PIN, LOW);
    ...
    if (packetSize == sizeof(LoRaPacket)) {
      ...
      if (xQueueSend(loraPacketQueue, &receivedPacket, (TickType_t)10) != pdPASS) {
        Serial.println("LoRa packet queue is full. Packet dropped.");
      }
    } else if (packetSize > 0) {
      // A packet of the wrong size was received. Flush it.
      Serial.printf("LoRa packet size mismatch. Expected %d, got %d. Flushing.\n", sizeof(LoRaPacket), packetSize);
...
void processLoRaQueue() {
  ...
  if (xQueueReceive(loraPacketQueue, &receivedPacket, (TickType_t)0) == pdPASS) {
    ...
    Serial.println("Processing LoRa packet from queue...");
    ...
    if (receivedPacket.groupID != allowedGroupID || receivedPacket.serialID != allowedSerialID) {
      Serial.println("LoRa packet ID mismatch.");
    ...
    if (loraPowerOptimizationEnabled) {
        if (lastPacketRssi > RSSI_TOO_STRONG) {
            ...
            if (transmitterTxPower > 2) { // Don't go below the minimum power
                transmitterTxPower--;
                Serial.printf("RSSI is strong (%d dBm). Requesting TX power decrease to %d.\n", lastPacketRssi, transmitterTxPower);
            ...
            if (transmitterTxPower < 20) { // Don't go above the maximum power
                transmitterTxPower++;
                Serial.printf("RSSI is weak (%d dBm). Requesting TX power increase to %d.\n", lastPacketRssi, transmitterTxPower);
                saveSettingsToEEPROM(); // Persist the new auto-adjusted power level
            }
        }
    }
    ...
    Serial.printf("Confirmed settings from transmitter -> Power: %d, ACK Attempts: %d\n", confirmedTxPower, confirmedAckAttempts);
    ...
    if (lastObservedValidTankLevel == TANK_INVALID) {
      ...
      Serial.println("Initial heartbeat received. Sequence check bypassed.");
    } else {
    ...
    if (incomingTankLevel != lastObservedValidTankLevel) {
        ...
        if (incomingTankLevel == TANK_INVALID) {
            ...
            Serial.print("Sensor Error: Invalid switch combination. Detail: ");
            Serial.println(sensorErrorDetailMessage);
    ...
    if (sensorError != SENSOR_ERROR_NONE && !sensorErrorBypassEnabled) {
        if (pumpRunning || waitingForPumpRelay) {
            Serial.println("CRITICAL: Stopping pump due to un-bypassed sensor error.");
            stopPump();
    ...
    if (pumpRunning && (pumpMode != MANUAL || manualModeSafetyLogicEnabled)) {
      ...
      if (dryRunLogicEnabled) {
        if (waitingForFlow && newSwitchState[0]) {
          flowOk = true;
          waitingForFlow = false;
          Serial.println("Flow detected. Dry run averted.");
          ...
          waitingForFlow = true;
          pumpStartTime = millis();
          Serial.println("Flow lost. Re-entering waiting for flow state.");
        }
      }
    }
  }
